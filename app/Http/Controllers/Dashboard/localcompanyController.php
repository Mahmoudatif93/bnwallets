...
class localcompanyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $Companies = Company::where(array('api'=>0,'kind'=>'local'))->when($request->search, function ($q) use ($request) {

            return $q->where('name','like', '%' .  $request->search . '%')
                ->orWhere('kind', 'like', '%' . $request->search . '%');
        })->latest()->paginate(5);
        
        return view('dashboard.localcompany.index', compact('Companies'));
    ...
    public function show($id)
    {
     ...
     return view('dashboard.localcompany.dubaiorderdetails', compact('cards'));
